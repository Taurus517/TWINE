...
const vector<int> delta_upper = {}; //
const vector<int>  key_upper = {}; //

const vector<int> delta_lower = {}; //
const vector<int>  key_lower = {}; //
...
const vector<int> RK_128 = {2, 3, 12, 15, 17, 18, 28, 31};

vector<int> Rot_128;
vector<int> Rot_128_inv;
void initRotations() {
    for (int i = 4; i < 32; i++) {
        Rot_128.push_back(i);
    }
    ...
    for (int i : {31, 28, 29, 30}) {
        Rot_128_inv.push_back(i);
    }
    for (int i = 0; i < 28; i++) {
        Rot_128_inv.push_back(i);
    }
...
pair<vector<vector<int>>, vector<int>> KeySchedule_128(vector<int> Ktemp, int R0, int R1) {
...
vector<vector<int>> KeySchedule_128_inv(vector<int> Ktemp, int R0, int R1) {
    vector<vector<int>> RKAlltemp;
...
vector<int> Enc(vector<int> Plain, vector<vector<int>> RKAlltemp, int R) {
...
vector<int> Dec(vector<int> Cipher, vector<vector<int>> RKAlltemp, int R) {
    int Rcount = 0;
...
void process_data(uint64_t start, uint64_t end){
    ...
    for (uint64_t count = start; count < end; count++){
        ...
        vector<vector<int>> RKAll_1 = KeySchedule_128(K1, r0, r0 + rm).first;
        vector<int> K_1 = KeySchedule_128(K1, r0, r0 + rm).second;
        ...
        vector<vector<int>> RKAll_2 = KeySchedule_128(K2, r0, r0 + rm).first;
        vector<int> K_2 = KeySchedule_128(K2, r0, r0 + rm).second;
        ...
        vector<vector<int>> RKAll_3 = KeySchedule_128_inv(K3, r0, r0 + rm);

        vector<int> K4(int(Key/4), 0);
        for (int i = 0; i < int(Key/4); i++) {
            K4[i] = K_2[i] ^ key_lower[i];
        }
        vector<vector<int>> RKAll_4 = KeySchedule_128_inv(K4, r0, r0 + rm);
        ...
        vector<int> C1 = Enc(P1, RKAll_1, rm);

        vector<int> C3(int(Block/4), 0);
        for (int i = 0; i < int(Block/4); i++) {
            C3[i] = C1[i] ^ delta_lower[i];
        }
        vector<int> P3 = Dec(C3, RKAll_3, rm);
        ...
        vector<int> C2 = Enc(P2, RKAll_2, rm);

        vector<int> C4(int(Block/4), 0);
        for (int i = 0; i < int(Block/4); i++) {
            C4[i] = C2[i] ^ delta_lower[i];
        }
        vector<int> P4 = Dec(C4, RKAll_4, rm);
...
int main() {
    ...
    uint64_t chunk_size = totalData / num_threads; 
    ...
    cout << "runTime: " << duration << " minute" << endl << endl;
